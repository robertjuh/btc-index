...
export abstract class LineChartComponent implements AfterViewInit {
  ...
  protected afterConfigure(): void {
    this.columns = this.maxX / 2;
  }

  private addSvgContainer(): void {
  ...
  private setBackgroundSizes(): void {
  ...
  private checkEndOfAnimation(i): void {
    if (i >= this.maxX + 1) {
